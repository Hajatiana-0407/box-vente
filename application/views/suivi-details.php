            ...
            <div class="stock_corps">
                ...
                <div class="modal fade" id="showDetails" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-xl" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title vente">Détails</h4>
                ...
                <h5 class="mb-4">Client : <?= $nom ?? '--' ?></h5>

                <form action="<?= base_url('suivi-client/details-filtre') ?>" method="post" class="mb-4 " id="searchform">
                    <!-- <div class="row"> -->
                    <input type="hidden" value="<?= $telClient ?>" name="telClient">
                ...
                <div >
                    <a href="<?= base_url('suivi-client') ?>" class="btn btn-info">
                        <i class="fas fa-reply"></i>
                        Retour
                    </a>
                </div>
                <div class="_tableau mt-2">
                    <table class="table">
                        ...
                        <tbody class="search-results">
                            ...
                            <?php for ($i = 0; $i < count($data); $i++) : ?>
                                <tr>
                                    ...
                                    <td><?= ($data[$i]->telClient == '') ? '...' :  $data[$i]->telClient  ?></td>
                                    <td><?= ($data[$i]->denom == '') ? '...' :  $data[$i]->denom  ?></td>
                                    <td><?= $data[$i]->prenomUser ?? 'Admin' ?></td>
                                    <td>
                                        <a href="#" data-toggle="modal" data-target="#showDetails" class="btn btn-secondary detail" data-idfacture="<?= $data[$i]->idfacture ?>" data-num="<?= $data[$i]->telClient ?>"><i class="fas fa-list"></i></a>
                                    </td>
                                </tr>
                            <?php endfor; ?>
                        </tbody>
                                ...
                                <ul class="pagination justify-content-center pagination-sm">
                                    <li class="page-item">
                                        ...
                                        <?php else :  ?>
                                            <?php if (isset($_POST['date_debut'])) :  ?>
                                                ...
                                                <form action="<?= base_url('suivi-client/details-filtre/' . ($current - 1)) ?>" method="post">
                                                    <input type="hidden" value="<?= $telClient ?>" name="telClient">
                                                    <input type="hidden" value="<?= $nom ?>" name="nom">
                                            ...
                                            <?php else :  ?>
                                                <form action='<?= base_url('suivi-client/details/' . ($current - 1)) ?>' method="post">
                                                    <input type="hidden" value="<?= $telClient ?>" name="telClient">
                                                    <input type="hidden" value="<?= $nom ?>" name="nom">
                                    ...
                                    <?php for ($i = 1; $i <= $nPages; $i++) :  ?>
                                        <li class="page-item <?= ($i == $current) ? 'active'  :  '' ?>">
                                            <?php if (isset($_POST['date_debut'])) :  ?>
                                                <form action="<?= base_url('suivi-client/details-filtre/' . ($i)) ?>" method="post">
                                                    <input type="hidden" value="<?= $telClient ?>" name="telClient">
                                                    <input type="hidden" value="<?= $nom ?>" name="nom">
                                                    <div class="row d-none">
                                                        <div class="col-4 row">
                                            ...
                                            <?php else :  ?>
                                                <form action="<?= base_url('suivi-client/details/' . $i) ?>" method="post">
                                                    <input type="hidden" value="<?= $telClient ?>" name="telClient">
                                                    <input type="hidden" value="<?= $nom ?>" name="nom">
                                                    <button class="page-link">
                                                        <?= $i ?>
